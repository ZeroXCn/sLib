...
public:
	...
	SVariant(void) :type_index_(typeid(void))
	{
	}
	...
	SVariant(SVariant<Types...>&& old) : type_index_(old.type_index_)
	{
		move(old.type_index_, &old.data_, &data_);
	}

	SVariant(const SVariant<Types...>& old) : type_index_(old.type_index_)
	{
		copy(old.type_index_, &old.data_, &data_);
	}

	SVariant& operator=(const SVariant& old)
	{
		copy(old.type_index_, &old.data_, &data_);
	...
	SVariant& operator=(SVariant&& old)
	{
		move(old.type_index_, &old.data_, &data_);
		type_index_ = old.type_index_;
		return *this;
	...
	class = typename std::enable_if<Contains<typename std::decay<T>::type, Types...>::value>::type>
		SVariant(T&& value) : type_index_(typeid(void))
	{
			destroy(type_index_, &data_);
